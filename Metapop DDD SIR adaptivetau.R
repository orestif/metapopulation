...
meta.DDD.SIR.transitions <- function(contact.matrix){
...
meta.DDD.SIR.ssa.tau <- function(n.simul, mat, par, init, t.end, thin=NA, tau.leap.par=list(epsilon=0.005)){
	foreach(1:n.simul, .inorder = F, .packages='adaptivetau') %dopar% 
	{
		sim <- ssa.adaptivetau(init,meta.DDD.SIR.transitions(mat),meta.DDD.SIR.Rates,sim.par(mat,par),tf=t.end,tl.params = tau.leap.par)
		if(is.finite(thin)){
...
meta.DDD.SIR.ssa.stats <- function(sim,n){
	I.col <- (n+2):(2*n+1)
...
meta.DDD.SIR.ode <- function(meta.mat,par,init,t.end,dt=0.005){
	ode(init,seq(0,t.end,dt),stoch.diff,sim.par(meta.mat,par),trans.mat=meta.DDD.SIR.transitions(meta.mat),rate.fun=meta.DDD.SIR.Rates)
}

# Use the birth-death deterministic model to calculate S(0) on a stable cycle in the absence of infection and migration.
meta.DDD.SIR.init <- function(par){
	np <- length(par$s)
	...
	foreach(i=1:np, .combine=c) %dopar%{
		optimize(function(x){
			init <- rep(0,3*np)
			init[i] <- x
			(meta.DDD.SIR.ode(meta.mat,par,init,t.end=1,dt=1)[2,i+1]-x)^2},
